...
#include "ifj2148.h"
#include "ifj2148_conf.h"

static int itv_fj2148_module_debug;
static int itv_fj2148_module_debug_callstack;

module_param_named(debug, itv_fj2148_module_debug, int, 0644);
module_param_named(debug_cs, itv_fj2148_module_debug_callstack, int, 0644);

MODULE_PARM_DESC(debug, "Turn on/off fj2148 debugging (default:off).");
MODULE_PARM_DESC(debug_cs, "Turn on/off fj2148 callstack debugging (default:off).");
...
#define dprintk(args...) \
	do { if(itv_fj2148_module_debug) { printk("[%s] ", __func__); printk(args); } } while (0)

#define DEBUG_CALLSTACK		\
	do { if(itv_fj2148_module_debug_callstack) { printk("[Call] ----> %s()\n", __func__); } } while (0);
...
static int itv_fj2148_readreg(itv_tuner_priv_t *p_priv, unsigned char reg, unsigned char *val)
{
	...
	if(i2c_transfer(p_priv->i2c_props.i2c, msg, 2) < 0) {
		wprintk("fj2148: I2C read failed\n");
		return FJ_RESULT_I2C_READ_FAILURE;
	}
	
	return FJ_RESULT_SUCCESS;
}

static int itv_fj2148_readregs(itv_tuner_priv_t *p_priv, const TDA18273_BitField_t* pBitField, unsigned char *val, tmbslFrontEndBusAccess_t eBusAccess)
{
	...
	if(RegAddr < TDA18273_REG_MAP_NB_BYTES)	{
		pRegData = (unsigned char *)(&(p_priv->regmap)) + RegAddr;
	} else {
		pRegData = &RegData;
	}
	
	if((eBusAccess & Bus_NoRead) == 0x00) {
		if(itv_fj2148_readreg(p_priv, RegAddr, pRegData) == FJ_RESULT_I2C_READ_FAILURE) {
			return FJ_RESULT_I2C_READ_FAILURE;
		}
	}
...
static int itv_fj2148_readregmap(itv_tuner_priv_t *p_priv, unsigned char reg, unsigned int len)
{
	int ret, i;
	unsigned char* pRegData = NULL;
	
	if((reg < TDA18273_REG_MAP_NB_BYTES) && ((reg + len) <= TDA18273_REG_MAP_NB_BYTES)) {
		pRegData = (unsigned char *)(&(p_priv->regmap)) + reg;
		
		for(i=0; i<len; i++) {
			/* Read data from TDA18273 */
			ret = itv_fj2148_readreg(p_priv, (reg + i), (pRegData + i));
		}
	}
...
static int itv_fj2148_writereg(itv_tuner_priv_t *p_priv, unsigned char reg, unsigned char val)
{
	...
	if(i2c_transfer(p_priv->i2c_props.i2c, &msg, 1) != 1) {
		eprintk("fj2148: I2C write failed\n");
		return FJ_RESULT_I2C_WRITE_FAILURE;
	}

	return FJ_RESULT_SUCCESS;
}

static int itv_fj2148_writeregs(itv_tuner_priv_t *p_priv, const TDA18273_BitField_t* pBitField, unsigned char val, tmbslFrontEndBusAccess_t eBusAccess)
{
	unsigned char RegAddr = 0;
	unsigned char RegData = 0;
	unsigned char RegMask = 0;
	...
	*pRegData |= val << pBitField->PositionInBits;
	...
	if((eBusAccess & Bus_NoWrite) == 0x00) {
		if(itv_fj2148_writereg(p_priv, RegAddr, *pRegData) == FJ_RESULT_I2C_READ_FAILURE) {
			return FJ_RESULT_I2C_READ_FAILURE;
		}
	}
...
static int itv_fj2148_writeregmap(itv_tuner_priv_t *p_priv, unsigned char reg, unsigned int len)
{
	int ret, i;
	unsigned char* pRegData = NULL;
	...
	if((reg < TDA18273_REG_MAP_NB_BYTES) && ((reg + len) <= TDA18273_REG_MAP_NB_BYTES)) {
		...
		for(i=0; i<len; i++) {
			...
			ret = itv_fj2148_writereg(p_priv, (reg + i), *(pRegData + i));
		}
	}
...
static void itv_fj2148_regall_debug(itv_tuner_priv_t *p_priv)
{
	int i;
	unsigned char val=0;
	...
	printk("\nFJ2148 #########################\n");
	for(i=0; i<0x45; i++) {
		itv_fj2148_readreg(p_priv, i, &val);
		printk("addr : 0x%02x  =>  data : 0x%02x\n", i, val);
	}
	printk("################################\n\n");
}

static int itv_fj2148_get_lock_status(itv_tuner_priv_t *p_priv, unsigned short *lock_status)
{
	int ret;
	unsigned char val=0;
	unsigned char val_lo=0;

	ret = itv_fj2148_readregs(p_priv, &gTDA18273_Reg_Power_state_byte_1__LO_Lock, &val_lo, Bus_RW);
	ret = itv_fj2148_readregs(p_priv, &gTDA18273_Reg_IRQ_status__IRQ_status, &val, Bus_RW);
	*lock_status = val & val_lo;

	dprintk("lock=0x%02X/0x%02x, lock_status=0x%x\n", val, val_lo, *lock_status);

	return 0;
}

static void itv_fj2148_debug_dump(itv_tuner_priv_t *p_priv)
{
...
//	DEBUG_CALLSTACK
	...
	itv_fj2148_get_lock_status(p_priv, &lock_status);
	dprintk("*** Lock status (0-No-signal, 1-Locked) = %d\n", lock_status);
}

static int itv_fj2148_pwr_state(itv_frontend_t *p_ifrontend, int pwr_state)
{
	...
	if(priv->power_state != pwr_state) {
		if(pwr_state == tmPowerOn) {
			...
			ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Power_Down_byte_2__PD_LNA, 0x1, Bus_RW);				/* PD LNA */
			ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Power_Down_byte_2__PD_Det1, 0x1, Bus_NoRead);			/* PD Detector AGC1 */
			ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_AGC1_byte_3__AGC1_loop_off, 0x1, Bus_RW);				/* AGC1 Detector loop off */
			ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Power_state_byte_2, TDA18273_SM_NONE, Bus_RW);
			ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Reference_Byte__Digital_Clock_Mode, 0x03, Bus_RW);		/* Set digital clock mode to sub-LO if normal mode is entered */
		} else if(pwr_state == tmPowerStandby) {
			/* Set TDA18273 power state to standby with Xtal ON */
			ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Reference_Byte__Digital_Clock_Mode, 0x00, Bus_RW);
			ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Power_state_byte_2, 0x02, Bus_RW);
		}
...
static int itv_fj2148_firstpass_lnapwr(itv_frontend_t *p_ifrontend)
{
	itv_tuner_priv_t *priv = p_ifrontend->ituner.priv;
	...
	/* PD LNA */
    ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Power_Down_byte_2__PD_LNA, 0x1, Bus_RW);
	/* PD Detector AGC1 */
	ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Power_Down_byte_2__PD_Det1, 0x1, Bus_NoRead);
	/* AGC1 Detector loop off */
	ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_AGC1_byte_3__AGC1_loop_off, 0x1, Bus_RW);
        
	return 0;
}

static int itv_fj2148_lastpass_lnapwr(itv_frontend_t *p_ifrontend)
{
	itv_tuner_priv_t *priv = p_ifrontend->ituner.priv;
	...
	/* Check if LNA is PD */
    ret = itv_fj2148_readregs(priv, &gTDA18273_Reg_Power_Down_byte_2__PD_LNA, &val, Bus_NoWrite);

	if(val == 1) {
		/* LNA is Powered Down, so power it up */
		/* Force gain to -10dB */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_AGC1_byte_3__AGC1_Gain, 0x0, Bus_RW);
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_AGC1_byte_3__Force_AGC1_gain, 0x1, Bus_NoRead);
		/* PD LNA */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Power_Down_byte_2__PD_LNA, 0x0, Bus_NoRead);
		/* Release LNA gain control */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_AGC1_byte_3__Force_AGC1_gain, 0x0, Bus_NoRead);
		/* PD Detector AGC1 */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Power_Down_byte_2__PD_Det1, 0x0, Bus_NoRead);
		/* AGC1 Detector loop off */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_AGC1_byte_3__AGC1_loop_off, 0x0, Bus_NoRead);
	}
    
	return 0;
}

static int itv_fj2148_llpwr_state(itv_frontend_t *p_ifrontend, int llpwr_state)
{
	itv_tuner_priv_t *priv = p_ifrontend->ituner.priv;
	...
	if(llpwr_state == TDA18273_PowerNormalMode) {
        ...
        /* PD LNA */
		ret = itv_fj2148_firstpass_lnapwr(p_ifrontend);
        ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Power_state_byte_2, TDA18273_SM_NONE, Bus_RW);
		/* Set digital clock mode to sub-LO if normal mode is entered */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Reference_Byte__Digital_Clock_Mode, 0x03, Bus_RW);
	...
	if(val) {
		...
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Reference_Byte__Digital_Clock_Mode, 0x00, Bus_RW);
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Power_state_byte_2, val, Bus_RW);
	}
	
	return 0;
}

static int itv_fj2148_check_calcpll(itv_frontend_t *p_ifrontend)
{
	itv_tuner_priv_t *priv = p_ifrontend->ituner.priv;
    ...
    ret = itv_fj2148_readregs(priv, &gTDA18273_Reg_Sigma_delta_byte_5__LO_Calc_Disable, &val, Bus_None);
    
    if(val != 0x00) {
        /* Enable Calc_PLL algorithm by putting PLL in automatic mode */
        ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Sigma_delta_byte_5__N_K_correct_manual, 0x00, Bus_None);
        ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Sigma_delta_byte_5__LO_Calc_Disable, 0x00, Bus_NoRead);
    } 
	
	return 0;
}

static int itv_fj2148_set_msm(itv_frontend_t *p_ifrontend, unsigned char val, int launch)
{
	itv_tuner_priv_t *priv = p_ifrontend->ituner.priv;
	int ret;
	
	/* Set state machine and Launch it */
	ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_MSM_byte_1, val, Bus_None);
	ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_MSM_byte_2__MSM_Launch, 0x01, Bus_None);
	ret = itv_fj2148_writeregmap(priv, gTDA18273_Reg_MSM_byte_1.Address, (launch ? 0x02 : 0x01));
	ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_MSM_byte_2__MSM_Launch, 0x00, Bus_None);
	
	return 0;
}

static int itv_fj2148_override_bandsplit(itv_frontend_t *p_ifrontend)
{
	itv_tuner_priv_t *priv = p_ifrontend->ituner.priv;
	...
	ret = itv_fj2148_readregs(priv, &gTDA18273_Reg_Bandsplit_Filter_byte__Bandsplit_Filter_SubBand, &Bandsplit, Bus_None);
	ret = itv_fj2148_readregs(priv, &gTDA18273_Reg_PowerSavingMode__PSM_Bandsplit_Filter, &uPrevPSM_Bandsplit_Filter, Bus_None);
	...
	if(uPrevPSM_Bandsplit_Filter != PSM_Bandsplit_Filter) {
		/* Write PSM bandsplit */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_PowerSavingMode__PSM_Bandsplit_Filter, PSM_Bandsplit_Filter, Bus_NoRead);
	}
    
	return 0;
}

static int itv_fj2148_set_standardmode(itv_frontend_t *p_ifrontend, TDA18273StandardMode_t StandardMode)
{
	itv_tuner_priv_t *priv = p_ifrontend->ituner.priv;
	int ret;
	...
	if((priv->pObj->StandardMode > TDA18273_StandardMode_Unknown) && (priv->pObj->StandardMode < TDA18273_StandardMode_Max)) {
		...
		/****************************************************************/
		/* Set LPF */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_IF_Byte_1__LP_Fc, priv->pObj->pStandard->LPF, Bus_None);
		/* Set LPF Offset */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_IF_Byte_1__LP_FC_Offset, priv->pObj->pStandard->LPF_Offset, Bus_None);
		/* Set DC_Notch_IF_PPF */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_IR_Mixer_byte_2__IF_Notch, priv->pObj->pStandard->DC_Notch_IF_PPF, Bus_None);
		/* Enable/disable HPF */
		if(priv->pObj->pStandard->IF_HPF == TDA18273_IF_HPF_Disabled) {
			ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_IR_Mixer_byte_2__Hi_Pass, 0x00, Bus_None);
		} else {
			ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_IR_Mixer_byte_2__Hi_Pass, 0x01, Bus_None);
		    /* Set IF HPF */
		    ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_IF_Byte_1__IF_HP_Fc, (UInt8)(priv->pObj->pStandard->IF_HPF - 1), Bus_None);
		}
		/* Set IF Notch */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_IF_Byte_1__IF_ATSC_Notch, priv->pObj->pStandard->IF_Notch, Bus_None);
		/* Set IF notch to RSSI */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_IF_AGC_byte__IFnotchToRSSI, priv->pObj->pStandard->IFnotchToRSSI, Bus_None);
				
		/****************************************************************/
		/* AGC TOP Settings                                             */
		/****************************************************************/
		/* Set AGC1 TOP I2C DN/UP */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_AGC1_byte_1__AGC1_TOP, priv->pObj->pStandard->AGC1_TOP_I2C_DN_UP, Bus_None);
		/* Set AGC1 Adapt TOP DN/UP */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_AGC1_byte_2__AGC1_Top_Mode_Val, priv->pObj->pStandard->AGC1_Adapt_TOP_DN_UP, Bus_None);
		/* Set AGC1 DN Time Constant */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_AGC1_byte_3__AGC1_Do_step, priv->pObj->pStandard->AGC1_DN_Time_Constant, Bus_None);
		/* Set AGC1 mode */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_AGC1_byte_2__AGC1_Top_Mode, priv->pObj->pStandard->AGC1_Mode, Bus_None);
		/* Set Range_LNA_Adapt */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Adapt_Top_byte__Range_LNA_Adapt, priv->pObj->pStandard->Range_LNA_Adapt, Bus_None);
		/* Set LNA_Adapt_RFAGC_Gv_Threshold */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Adapt_Top_byte__Index_K_LNA_Adapt, priv->pObj->pStandard->LNA_Adapt_RFAGC_Gv_Threshold, Bus_None);
		/* Set AGC1_Top_Adapt_RFAGC_Gv_Threshold */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Adapt_Top_byte__Index_K_Top_Adapt, priv->pObj->pStandard->AGC1_Top_Adapt_RFAGC_Gv_Threshold, Bus_None);
		/* Set AGC2 TOP DN/UP */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_AGC2_byte_1__AGC2_TOP, priv->pObj->pStandard->AGC2_TOP_DN_UP, Bus_None);
		/* Set AGC2 DN Time Constant */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_RF_Filters_byte_3__AGC2_Do_step, priv->pObj->pStandard->AGC2_DN_Time_Constant, Bus_None);
		/* Set AGC4 TOP DN/UP */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_IR_Mixer_byte_1__IR_Mixer_Top, priv->pObj->pStandard->AGC4_TOP_DN_UP, Bus_None);
		/* Set AGC5 TOP DN/UP */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_AGC5_byte_1__AGC5_TOP, priv->pObj->pStandard->AGC5_TOP_DN_UP, Bus_None);
		/* Set AGC3_Top_Adapt_Algorithm */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_RF_AGC_byte__PD_AGC_Adapt3x, priv->pObj->pStandard->AGC3_Top_Adapt_Algorithm, Bus_None);
		/* Set AGC Overload TOP */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Vsync_Mgt_byte__AGC_Ovld_TOP, priv->pObj->pStandard->AGC_Overload_TOP, Bus_None);
		/* Set Adapt TOP 34 Gain Threshold */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_RFAGCs_Gain_byte_1__TH_AGC_Adapt34, priv->pObj->pStandard->TH_AGC_Adapt34, Bus_None);
		/* Set RF atten 3dB */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_W_Filter_byte__RF_Atten_3dB, priv->pObj->pStandard->RF_Atten_3dB, Bus_None);
		/* Set IF Output Level */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_IF_AGC_byte__IF_level, priv->pObj->pStandard->IF_Output_Level, Bus_None);
		/* Set S2D gain */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_IR_Mixer_byte_1__S2D_Gain, priv->pObj->pStandard->S2D_Gain, Bus_None);
		/* Set Negative modulation, write into register directly because vsync_int bit is checked afterwards */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Vsync_byte__Neg_modulation, priv->pObj->pStandard->Negative_Modulation, Bus_RW);
		
		/****************************************************************/
		/* GSK Settings                                                 */
		/****************************************************************/
		/* Set AGCK Step */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_AGCK_byte_1__AGCK_Step, priv->pObj->pStandard->AGCK_Steps, Bus_None);
		/* Set AGCK Time Constant */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_AGCK_byte_1__AGCK_Mode, priv->pObj->pStandard->AGCK_Time_Constant, Bus_None);
		/* Set AGC5 HPF */
		...
		if(priv->pObj->pStandard->AGC5_HPF == TDA18273_AGC5_HPF_Enabled) {
			...
			ret = itv_fj2148_readregs(priv, &gTDA18273_Reg_Vsync_byte__Vsync_int, &checked, Bus_RW);
		...
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_AGC5_byte_1__AGC5_Ana, wantedValue, Bus_None);
		/* Set Pulse Shaper Disable */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_AGCK_byte_1__Pulse_Shaper_Disable, priv->pObj->pStandard->Pulse_Shaper_Disable, Bus_None);

		/****************************************************************/
		/* H3H5 Settings                                                */
		/****************************************************************/
		/* Set VHF_III_Mode */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_W_Filter_byte__VHF_III_mode, priv->pObj->pStandard->VHF_III_Mode, Bus_None);
		
		/****************************************************************/
		/* PLL Settings                                                 */
		/****************************************************************/
		/* Set LO_CP_Current */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_CP_Current_byte__LO_CP_Current, priv->pObj->pStandard->LO_CP_Current, Bus_None);
		
		/****************************************************************/
		/* IF Settings                                                  */
		/****************************************************************/
		/* Set IF */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_IF_Frequency_byte__IF_Freq, (UInt8)((priv->pObj->pStandard->IF - priv->pObj->pStandard->CF_Offset)/50000), Bus_None);

		/****************************************************************/
		/* MISC Settings                                                */
		/****************************************************************/
		/* Set PD Underload */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Misc_byte__PD_Underload, priv->pObj->pStandard->PD_Underload, Bus_None);

		/****************************************************************/
		/* Update Registers                                             */
		/****************************************************************/
		/* Write AGC1_byte_1 (0x0C) to IF_Byte_1 (0x15) Registers */
		ret = itv_fj2148_writeregmap(priv, gTDA18273_Reg_AGC1_byte_1.Address, TDA18273_REG_DATA_LEN(gTDA18273_Reg_AGC1_byte_1, gTDA18273_Reg_IF_Byte_1));
		/* Write IF_Frequency_byte (0x17) Register */
		ret = itv_fj2148_writeregmap(priv, gTDA18273_Reg_IF_Frequency_byte.Address, 1);
		/* Write Adapt_Top_byte (0x1F) Register */
		ret = itv_fj2148_writeregmap(priv, gTDA18273_Reg_Adapt_Top_byte.Address, 1);
		/* Write Vsync_byte (0x20) to RFAGCs_Gain_byte_1 (0x24) Registers */
		ret = itv_fj2148_writeregmap(priv, gTDA18273_Reg_Vsync_byte.Address, TDA18273_REG_DATA_LEN(gTDA18273_Reg_Vsync_byte, gTDA18273_Reg_RFAGCs_Gain_byte_1));
		/* Write RF_Filters_byte_3 (0x2D) Register */
		ret = itv_fj2148_writeregmap(priv, gTDA18273_Reg_RF_Filters_byte_3.Address, 1);
		/* Write CP_Current_byte (0x2F) Register */
		ret = itv_fj2148_writeregmap(priv, gTDA18273_Reg_CP_Current_byte.Address, 1);
		/* Write Misc_byte (0x38) Register */
		ret = itv_fj2148_writeregmap(priv, gTDA18273_Reg_Misc_byte.Address, 1);
	}
...
static int itv_fj2148_calculate_postdivandprescaler(unsigned int LO, int growingOrder, unsigned char* PostDiv, unsigned char* Prescaler)
{
	int ret = 0;
...
static int itv_fj2148_find_postdivandprescalerwithbettermargin(unsigned int LO, unsigned char* PostDiv, unsigned char* Prescaler)
{
	int ret;
	
	unsigned char PostDivGrowing;
	unsigned char PrescalerGrowing;
	unsigned char PostDivDecreasing;
	unsigned char PrescalerDecreasing;
	...
	ret = itv_fj2148_calculate_postdivandprescaler(LO, 1, &PostDivGrowing, &PrescalerGrowing);
	if(ret != -1) {
		/* Calculate corresponding FVCO value in kHz */
		FCVOGrowing = LO * PrescalerGrowing * PostDivGrowing;
	}
	
	ret = itv_fj2148_calculate_postdivandprescaler(LO, 0, &PostDivDecreasing, &PrescalerDecreasing);
	if(ret != -1) {
		/* Calculate corresponding FVCO value in kHz */
...
static int itv_fj2148_calculate_nintkint(unsigned int LO, unsigned char PostDiv, unsigned char Prescaler, unsigned int* NInt, unsigned int* KInt)
{
...
static int itv_fj2148_set_pll(itv_frontend_t *p_ifrontend)
{
	itv_tuner_priv_t *priv = p_ifrontend->ituner.priv;
	...
	ret = itv_fj2148_find_postdivandprescalerwithbettermargin(LO, &PostDiv, &Prescaler);
	...
	if(ret != -1) {
		...
		switch(PostDiv)	{
			...
			default:	printk("%s *PostDiv value is wrong.", __func__);	break;
		}
		
		/* Affect register map without writing into IC */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Main_Post_Divider_byte__LOPostDiv, LOPostDiv, Bus_None);
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Main_Post_Divider_byte__LOPresc, Prescaler, Bus_None);
		/* Calculate N & K values of the PLL */
		ret = itv_fj2148_calculate_nintkint(LO, PostDiv, Prescaler, &N_int, &K_int);
		/* Affect registers map without writing to IC */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Sigma_delta_byte_4__LO_Frac_0, (UInt8)(K_int & 0xFF), Bus_None);
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Sigma_delta_byte_3__LO_Frac_1, (UInt8)((K_int >> 8) & 0xFF), Bus_None);
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Sigma_delta_byte_2__LO_Frac_2, (UInt8)((K_int >> 16) & 0xFF), Bus_None);
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Sigma_delta_byte_1__LO_Int, (UInt8)(N_int & 0xFF), Bus_None);
		/* Force manual selection mode : 0x3 at @0x56 */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Sigma_delta_byte_5__N_K_correct_manual, 0x01, Bus_None);
		/* Force manual selection mode : 0x3 at @0x56 */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Sigma_delta_byte_5__LO_Calc_Disable, 0x01, Bus_None);
		/* Write bytes Main_Post_Divider_byte (0x51) to Sigma_delta_byte_5 (0x56) */
		ret = itv_fj2148_writeregmap(priv, gTDA18273_Reg_Main_Post_Divider_byte.Address, TDA18273_REG_DATA_LEN(gTDA18273_Reg_Main_Post_Divider_byte, gTDA18273_Reg_Sigma_delta_byte_5));
	}
		
	return 0;
}

static int itv_fj2148_override_icp(itv_frontend_t *p_ifrontend, unsigned int uRF)
{
	itv_tuner_priv_t *priv = p_ifrontend->ituner.priv;
	...
	ret = itv_fj2148_readregs(priv, &gTDA18273_Reg_Main_Post_Divider_byte, &LOPostdiv, Bus_RW);
	/* PostDiv */
	ret = itv_fj2148_readregs(priv, &gTDA18273_Reg_Main_Post_Divider_byte__LOPostDiv, &LOPostdiv, Bus_NoRead);
	/* Prescaler */
	ret = itv_fj2148_readregs(priv, &gTDA18273_Reg_Main_Post_Divider_byte__LOPresc, &LOPrescaler, Bus_NoRead);
	/* IF */
	ret = itv_fj2148_readregs(priv, &gTDA18273_Reg_IF_Frequency_byte__IF_Freq, &ProgIF, Bus_NoRead);
	/* Decode IF */
	uIF = ProgIF*50000;
	/* Decode PostDiv */
	switch(LOPostdiv) {
	...
	if(ret != -1) {
		...
		/* Get ICP_bypass bit */
		ret = itv_fj2148_readregs(priv, &gTDA18273_Reg_Charge_pump_byte__ICP_Bypass, &uICPBypass, Bus_RW);
		/* Get ICP */
		ret = itv_fj2148_readregs(priv, &gTDA18273_Reg_Charge_pump_byte__ICP, &uPrevICP, Bus_NoRead);

		if(uICPBypass == 0x0 || uPrevICP != ICP) {
			/* Set ICP_bypass bit */
			ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Charge_pump_byte__ICP_Bypass, 0x01, Bus_None);
			/* Set ICP */
			ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Charge_pump_byte__ICP, ICP, Bus_None);
			/* Write Charge_pump_byte register */
			ret = itv_fj2148_writeregmap(priv, gTDA18273_Reg_Charge_pump_byte.Address, 1);
		}
	}
...
static int itv_fj2148_override_digitalclock(itv_frontend_t *p_ifrontend, unsigned int uRF)
{
	itv_tuner_priv_t *priv = p_ifrontend->ituner.priv;
	...
	/* Read Current IF */
	ret = itv_fj2148_readregs(priv, &gTDA18273_Reg_IF_Frequency_byte__IF_Freq, &uProgIF, Bus_NoWrite);
	/* Read Current Digital Clock */
	ret = itv_fj2148_readregs(priv, &gTDA18273_Reg_Reference_Byte__Digital_Clock_Mode, &uPrevDigClock, Bus_NoWrite);
	/* LO = RF + IF */
	...
	if(uPrevDigClock != uDigClock) {
		...
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Reference_Byte__Digital_Clock_Mode, uDigClock, Bus_NoRead);
	}
    
    return 0;
}

static int itv_fj2148_set_rffreq(itv_frontend_t *p_ifrontend)
{
	itv_tuner_priv_t *priv = p_ifrontend->ituner.priv;
	...
	if(uRF < priv->pObj->pStandard->Freq_Start_LTE) {
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_RF_AGC_byte__RFAGC_Top, priv->pObj->pStandard->AGC3_TOP_I2C_Low_Band, Bus_RW);
	} else {
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_RF_AGC_byte__RFAGC_Top, priv->pObj->pStandard->AGC3_TOP_I2C_High_Band, Bus_RW);
	}
	/* Set AGC3 Adapt TOP */
	if(uRF < priv->pObj->pStandard->Freq_Start_LTE) {
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_RF_AGC_byte__RFAGC_Adapt_TOP, priv->pObj->pStandard->AGC3_Adapt_TOP_Low_Band, Bus_NoRead);
	} else {
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_RF_AGC_byte__RFAGC_Adapt_TOP, priv->pObj->pStandard->AGC3_Adapt_TOP_High_Band, Bus_NoRead);
	}
	/* Set IRQ_clear */
	ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_IRQ_clear, TDA18273_IRQ_Global|TDA18273_IRQ_SetRF, Bus_NoRead);
	/* Set RF */
	uRFLocal = (uRF + 500) / 1000;
	
	ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_RF_Frequency_byte_1__RF_Freq_1, (unsigned char)((uRFLocal & 0x00FF0000) >> 16), Bus_None);
	ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_RF_Frequency_byte_2__RF_Freq_2, (unsigned char)((uRFLocal & 0x0000FF00) >> 8), Bus_None);
	ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_RF_Frequency_byte_3__RF_Freq_3, (unsigned char)(uRFLocal & 0x000000FF), Bus_None);
	ret = itv_fj2148_writeregmap(priv, gTDA18273_Reg_RF_Frequency_byte_1.Address, TDA18273_REG_DATA_LEN(gTDA18273_Reg_RF_Frequency_byte_1, gTDA18273_Reg_RF_Frequency_byte_3));
	/* Set state machine and Launch it */
	ret = itv_fj2148_set_msm(p_ifrontend, TDA18273_MSM_SetRF, 0x1);
	...
	ret = itv_fj2148_set_pll(p_ifrontend);
	/* Override ICP */
	ret = itv_fj2148_override_icp(p_ifrontend, priv->pObj->uProgRF);
	/* Override Digital Clock */
	if(ret != -1) {
		ret = itv_fj2148_override_digitalclock(p_ifrontend, priv->pObj->uProgRF);
	}

	return 0;
}

static int itv_fj2148_set_rf(itv_frontend_t *p_ifrontend, unsigned int uRF)
{
	itv_tuner_priv_t *priv = p_ifrontend->ituner.priv;
	...
	itv_fj2148_llpwr_state(p_ifrontend, TDA18273_PowerNormalMode);
	itv_fj2148_lastpass_lnapwr(p_ifrontend);
	itv_fj2148_check_calcpll(p_ifrontend);
	itv_fj2148_override_bandsplit(p_ifrontend);
	itv_fj2148_set_rffreq(p_ifrontend);
	
	return 0;
}

static int itv_fj2148_hw_reset(itv_frontend_t *p_ifrontend)
{
	itv_tuner_priv_t *priv = p_ifrontend->ituner.priv;
	int ret;
	
	itv_fj2148_pwr_state(p_ifrontend, tmPowerStandby);
	itv_fj2148_llpwr_state(p_ifrontend, TDA18273_PowerNormalMode);
	...
	ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Power_Down_byte_3, 0x03, Bus_RW);
	ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Power_Down_byte_3, 0x00, Bus_NoRead);
	/* Set power state on */
	//ret = itv_fj2148_llpwr_state(p_ifrontend, TDA18273_PowerNormalMode);
	...
	if(1) {		//if (priv->bBufferMode == False)
		...
		/* Set IRQ_clear */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_IRQ_clear, TDA18273_IRQ_Global|TDA18273_IRQ_XtalCal|TDA18273_IRQ_HwInit|TDA18273_IRQ_IrCal, Bus_NoRead);
		/* Launch XTALCAL */
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_MSM_byte_2__XtalCal_Launch, 0x01, Bus_NoRead);
		ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_MSM_byte_2__XtalCal_Launch, 0x00, Bus_None);
	...
	/* Read all bytes */
	ret = itv_fj2148_readregmap(priv, 0x00, TDA18273_REG_MAP_NB_BYTES);
	/* Check if Calc_PLL algorithm is in automatic mode */
	ret = itv_fj2148_check_calcpll(p_ifrontend);
	/* Up_Step_Ovld: POR = 1 -> 0 */
	ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Vsync_Mgt_byte__Up_Step_Ovld, 0x00, Bus_NoRead);
	/* PLD_CC_Enable: POR = 1 -> 0 */
	ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_RFAGCs_Gain_byte_1__PLD_CC_Enable, 0x00, Bus_NoRead);
	/* RFCAL_Offset0 : 0 */
	ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_RF_Cal_byte_1__RFCAL_Offset_Cprog0, 0x00, Bus_None);
	/* RFCAL_Offset1 : 0 */
	ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_RF_Cal_byte_1__RFCAL_Offset_Cprog1, 0x00, Bus_None);
	/* RFCAL_Offset2 : 0 */
	ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_RF_Cal_byte_1__RFCAL_Offset_Cprog2, 0x00, Bus_None);
	/* RFCAL_Offset3 : 0 */
	ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_RF_Cal_byte_1__RFCAL_Offset_Cprog3, 0x00, Bus_None);
	/* RFCAL_Offset4 : 3 */
	ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_RF_Cal_byte_2__RFCAL_Offset_Cprog4, 0x03, Bus_None);
	/* RFCAL_Offset5 : 0 */
	ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_RF_Cal_byte_2__RFCAL_Offset_Cprog5, 0x00, Bus_None);
	/* RFCAL_Offset6 : 3 */
	ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_RF_Cal_byte_2__RFCAL_Offset_Cprog6, 0x03, Bus_None);
	/* RFCAL_Offset7 : 3 */
	ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_RF_Cal_byte_2__RFCAL_Offset_Cprog7, 0x03, Bus_None);
	/* RFCAL_Offset8 : 1 */
	ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_RF_Cal_byte_3__RFCAL_Offset_Cprog8, 0x01, Bus_None);
	/* Write RF_Cal_byte_1 (0x27) to RF_Cal_byte_3 (0x29) Registers */
	ret = itv_fj2148_writeregmap(priv, gTDA18273_Reg_RF_Cal_byte_1.Address, TDA18273_REG_DATA_LEN(gTDA18273_Reg_RF_Cal_byte_1, gTDA18273_Reg_RF_Cal_byte_3));
	/* PLD_Temp_Enable: POR = 1 -> 0 */
	ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_RFAGCs_Gain_byte_1__PLD_Temp_Enable, 0x00, Bus_NoRead);
	/* Power Down Vsync Management: POR = 0 -> 1 */
	ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_Vsync_Mgt_byte__PD_Vsync_Mgt, 0x01, Bus_NoRead);
	/* Set IRQ_clear */
	ret = itv_fj2148_writeregs(priv, &gTDA18273_Reg_IRQ_clear, TDA18273_IRQ_Global|TDA18273_IRQ_HwInit|TDA18273_IRQ_IrCal, Bus_NoRead);
	/* Set state machine (all CALs except IRCAL) and Launch it */
	ret = itv_fj2148_set_msm(p_ifrontend, TDA18273_MSM_HwInit, 0x1);
	...
	ret = itv_fj2148_set_msm(p_ifrontend, TDA18273_MSM_IrCal, 0x1);
	/* State reached after 500 ms max, 10 ms step due to CAL ~ 30ms */
	//ret = iTDA18273_WaitIRQ(priv, 500, 10, TDA18273_IRQ_IrCal);
	mdelay(500);

	itv_fj2148_llpwr_state(p_ifrontend, TDA18273_PowerStandbyWithXtalOn);

	return 0;
}

static int itv_fj2148_hw_init(itv_frontend_t *p_ifrontend)
{
	itv_fj2148_pwr_state(p_ifrontend, tmPowerOn);
	
	itv_fj2148_hw_reset(p_ifrontend);

	return 0;
}

static int itv_fj2148_init(itv_frontend_t *p_ifrontend)
{
	itv_tuner_priv_t *priv = p_ifrontend->ituner.priv;
	int ret;
	unsigned char val=0;
	...
	ret = itv_fj2148_readregs(priv, &gTDA18273_Reg_ID_byte_1__Ident_1, &val, Bus_RW);
	id = val << 8;
	ret = itv_fj2148_readregs(priv, &gTDA18273_Reg_ID_byte_2__Ident_2, &val, Bus_RW);
	id |= val;
	
	if(ret == FJ_RESULT_SUCCESS) {
		if(id == FJ_DEVICE_TYPE) {
			ret = itv_fj2148_readregs(priv, &gTDA18273_Reg_ID_byte_3__Major_rev, &major, Bus_RW);
			ret = itv_fj2148_readregs(priv, &gTDA18273_Reg_ID_byte_3__Minor_rev, &minor, Bus_RW);
			printk("FJ2148 id:%d / ver:%d.%d\n", id, major, minor);
		} else {
			return -1;
		}
	} else {
	...
	itv_fj2148_hw_init(p_ifrontend);

	if(itv_fj2148_module_debug)
		itv_fj2148_debug_dump(priv);

	return 0;                        
} 

static int itv_fj2148_deinit(itv_frontend_t *p_ifrontend)
{
	itv_tuner_priv_t *priv = p_ifrontend->ituner.priv;
...
static int itv_fj2148_set_params(itv_frontend_t *p_ifrontend, itv_frontend_parameters_t *params)
{                                    
	itv_tuner_priv_t *priv = p_ifrontend->ituner.priv;
...
//	DEBUG_CALLSTACK                  
	...
	itv_fj2148_set_standardmode(p_ifrontend, TDA18273_QAM_6MHz);

	//20111130 : fj2148 ch freq 설정 시 if 만큼 +offset을 주어야 함.
	priv->freq_hz += p_ifrontend->ituner.priv->pObj->pStandard->IF;

	itv_fj2148_set_rf(p_ifrontend, priv->freq_hz);

	itv_fj2148_debug_dump(priv);
	//itv_fj2148_regall_debug(priv);

	return 0;
}

static int itv_fj2148_set_analog_params(itv_frontend_t *p_ifrontend, itv_frontend_parameters_t *params)
{
	itv_tuner_priv_t *priv = p_ifrontend->ituner.priv;
...
static int itv_fj2148_get_frequency(itv_frontend_t *p_ifrontend, unsigned int *freq)
{
	itv_tuner_priv_t *priv = p_ifrontend->ituner.priv;
...
static int itv_fj2148_get_bandwidth(itv_frontend_t *p_ifrontend, unsigned int *bw)
{
	itv_tuner_priv_t *priv = p_ifrontend->ituner.priv;
...
static int itv_fj2148_get_status(itv_frontend_t *p_ifrontend, unsigned int *status)
{
	unsigned short lock_status = 0;

	itv_tuner_priv_t *priv = p_ifrontend->ituner.priv;

	itv_fj2148_get_lock_status(priv, &lock_status);
...
static itv_fj2148_config_t fj2148_config = {
	.i2c_address      = 0x60,		//datasheet 상에는 AS_XTSEL의 voltage level에 따라 0xc0 or 0xc6 이지만 i2c-tcc 내에서 1bit shift(<<) 시키므로 shift 이전 값 사용.
	.if_freq          = 44000000,
};

static const itv_tuner_t fj2148_tuner = {
	.info = {
		.name           = "NuTune FJ2148",
	...
	.init				= itv_fj2148_init,
	.set_params			= itv_fj2148_set_params,
	.set_analog_params	= itv_fj2148_set_analog_params,
	.get_frequency		= itv_fj2148_get_frequency,
	.get_bandwidth		= itv_fj2148_get_bandwidth,
	.get_status			= itv_fj2148_get_status
};

static int itv_fj2148_activate(itv_object_t *p_this)
{	
	...
	priv->i2c_props.addr 	= fj2148_config.i2c_address;
	...
	priv->bandwidth	= BANDWIDTH_6_MHZ;
	priv->if_freq	= fj2148_config.if_freq;
	priv->power_state	= tmPowerOff;

	memcpy(&ifrontend->ituner, &fj2148_tuner, sizeof(itv_tuner_t));
	ifrontend->ituner.priv = priv;

	if(itv_fj2148_init(ifrontend) != 0)
	{
...
static void itv_fj2148_deactivate(itv_object_t *p_this)
{
	itv_frontend_t *ifrontend = (itv_frontend_t *)p_this;
	itv_platform_frontend_operations_t *fe_ops = ifrontend->fe_ops;
	...
	if(itv_fj2148_deinit(ifrontend) != 0) {
	}
...
itv_module_t ifj2148_module = {
	.psz_module_name = "fj2148", 
	.psz_capability = "ituner", 
	.i_score = 20, 
	.pf_activate = itv_fj2148_activate,
	.pf_deactivate = itv_fj2148_deactivate,
	.next = NULL, 
	.parent = NULL, 
	.submodule = NULL, 
	.i_submodules = 0
...
static int __init itv_fj2148_module_init(void)
{
	int retval;

	DEBUG_CALLSTACK

	if((retval = itv_module_register(&ifj2148_module)) != 0)
	{
		eprintk("itv_module_register() is failed\n");
		goto out;
	}

	dprintk("iTV fj2148 module register success\n");

out:
	return retval;
}

static void __exit itv_fj2148_module_exit(void)
{
	DEBUG_CALLSTACK
		
	itv_module_unregister(&ifj2148_module);

	dprintk("iTV fj2148 module unregister success\n");
}

module_init(itv_fj2148_module_init);
module_exit(itv_fj2148_module_exit);
