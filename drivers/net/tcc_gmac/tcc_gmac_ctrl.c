...
void tcc_gmac_enable_dma_transmission(unsigned long ioaddr)
{
	writel(1, ioaddr + DMA_XMT_POLL_DEMAND);
}

void tcc_gmac_enable_dma_irq(unsigned long ioaddr)
{
	writel(DMA_INTR_DEFAULT_MASK, ioaddr + DMA_INTR_ENA);
}

void tcc_gmac_disable_dma_irq(unsigned long ioaddr)
{
	writel(0, ioaddr + DMA_INTR_ENA);
}

void tcc_gmac_dma_start_tx(unsigned long ioaddr)
{
...
void tcc_gmac_dma_stop_tx(unsigned long ioaddr)
{
	u32 value = readl(ioaddr + DMA_CONTROL);
	value &= ~DMA_CONTROL_ST;
	writel(value, ioaddr + DMA_CONTROL);
...
void tcc_gmac_dma_start_rx(unsigned long ioaddr)
{
	u32 value = readl(ioaddr + DMA_CONTROL);
...
void tcc_gmac_dma_stop_rx(unsigned long ioaddr)
{
	u32 value = readl(ioaddr + DMA_CONTROL);
	value &= ~DMA_CONTROL_SR;
	writel(value, ioaddr + DMA_CONTROL);
...
int tcc_gmac_dma_interrupt(unsigned long ioaddr, struct tcc_gmac_extra_stats *x)
{
	int ret = 0;
...
void tcc_gmac_set_mac_addr(unsigned long ioaddr, u8 addr[6],
...
void tcc_gmac_get_mac_addr(unsigned long ioaddr, unsigned char *addr,
			 unsigned int high, unsigned int low)
{
...
static int tcc_gmac_dma_init(unsigned long ioaddr, int pbl, u32 dma_tx,
			      u32 dma_rx)
{
...
static void tcc_gmac_flush_tx_fifo(unsigned long ioaddr)
{
...
static void tcc_gmac_dma_operation_mode(unsigned long ioaddr, int txmode,
				    int rxmode)
{
	u32 csr6 = readl(ioaddr + DMA_CONTROL);
...
static void tcc_gmac_dump_dma_regs(unsigned long ioaddr)
{
	int i;
...
static void tcc_gmac_core_init(unsigned long ioaddr)
{
...
static void tcc_gmac_dump_regs(unsigned long ioaddr)
{
	int i;
...
static void tcc_gmac_set_umac_addr(unsigned long ioaddr, unsigned char *addr,
				unsigned int reg_n)
{
...
static void tcc_gmac_get_umac_addr(unsigned long ioaddr, unsigned char *addr,
				unsigned int reg_n)
{
...
static void tcc_gmac_set_filter(struct net_device *dev)
{
	...
	CTRL_DBG(KERN_INFO "%s: # mcasts %d, # unicast %d\n",
		__func__, netdev_mc_count(dev) , dev->uc_count);
...
static void tcc_gmac_flow_ctrl(unsigned long ioaddr, unsigned int duplex,
...
static void tcc_gmac_pmt(unsigned long ioaddr, unsigned long mode)
{
	unsigned int pmt = 0;
...
static void tcc_gmac_irq_status(unsigned long ioaddr)
{
...
struct tcc_gmac_dma_ops tcc_gmac_dma_ops = {
	.init = tcc_gmac_dma_init,
	.dump_regs = tcc_gmac_dump_dma_regs,
	.dma_mode = tcc_gmac_dma_operation_mode,
//	.dma_diagnostic_fr = tcc_gmac_dma_diagnostic_fr,
	.enable_dma_transmission = tcc_gmac_enable_dma_transmission,
	.enable_dma_irq = tcc_gmac_enable_dma_irq,
	.disable_dma_irq = tcc_gmac_disable_dma_irq,
	.start_tx = tcc_gmac_dma_start_tx,
	.stop_tx = tcc_gmac_dma_stop_tx,
	.start_rx = tcc_gmac_dma_start_rx,
	.stop_rx = tcc_gmac_dma_stop_rx,
	.dma_interrupt = tcc_gmac_dma_interrupt,
};
...
struct mac_device_info *tcc_gmac_setup(unsigned long ioaddr)
{
	...
	mac->clk_rate = GMII_CLK_RANGE_100_150M;
