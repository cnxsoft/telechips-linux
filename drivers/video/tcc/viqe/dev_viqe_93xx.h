...
typedef struct {
	...
	uint	di_di_mode_two_lbufs			:	 1;				// 14
	uint	di_mode_two_lbufs_auto_disable	:	 1;				// 15
	...
	uint									:	 3;				// 21:23
	uint	r_ctrl_mode_two_lbufs			:	 1;				// 24
	uint	ctrl_mode_two_lbufs				:	 1;				// 25
	uint									:	 6;				// 26:31
}	TOP_CTRL;
...
typedef struct 	{
	...
	uint	frmupdate_ctrl_mode_two_lbufs_disable	:	 1;			// 08
	uint	frmupdate_ctrl_ext_hpf_en_disable		:	 1;			// 09
	uint	priority_ext_hpf				:	 1;					// 10
	uint									:	 1;					// 11
...
// ---------------------------------------------------------------------
typedef	struct	{
	uint	spatial_jaggycheck				:	 1;
	uint	spatio_temp_jaggycheck			:	 1;
	uint	temporal_jaggycheck				:	 1;
	uint									:	 1;
...
typedef struct 	{
	...
	uint	dm_thres_c_sad					:	 8;
	uint	dm_thres_c_pixel				:	 8;
}	DI_ENGINE0;
...
typedef struct	{
	...
	uint	gen_thres_jaggy_div_half		:	 1;					// 02
	uint	edi_glmode_disable				:	 1;					// 03
	//uint									:	 4;					// 04:07
	uint	dir_length_type					:	 4;					// 04:07
	uint	edi_leftref_disable				:	 1;					// 08
	uint	edi_leftref_len					:	 5;					// 09:13
	uint	edi_leftref_type				:	 2;					// 14:15
...
typedef struct	{
	uint	edi_dir_adap_disable			:	 1;					// 00
	uint	edi_dir_early_disable			:	 1;					// 01
	//uint									:	 6;					// 02:07
	uint	edi_opp_dir_disable				:	 1;					// 02
	uint	edi_vert_err_ratio				:	 5;					// 03:07
	uint	edi_thres_dir_early				:	12;					// 08:19
	uint	edi_thres_dir_adap				:	12;					// 20:31
}	DI_ENGINE2;

typedef union	{
	uint			nDI_ENGINE2;
	DI_ENGINE2		bDI_ENGINE2;
}	DI_ENGINE2_U;

typedef struct	{
	uint	dm_stati_thres_disable			:	 1;					// 00
	uint									:	 7;					// 01:07
	uint	dm_stati_thres_mul				:	12;					// 08:19
	uint	dm_stati_thres_weight			:	12;					// 20:31
}	DI_ENGINE3;

typedef union	{
	uint			nDI_ENGINE3;
	DI_ENGINE3		bDI_ENGINE3;
}	DI_ENGINE3_U;
...
typedef struct	{
	...
	uint	dm_mode_c						:	 1;					// 01
	uint									:	 2;					// 02:03
	uint	dm_hpf_frm_thres				:	 4;					// 04:07
	uint	dm_hpf_fld_thres				:	 4;					// 08:11
	uint	dm_hpf_var_d_thres				:	 4;					// 12:15
	uint	dm_dyn_var_d_thres				:	 4;					// 16:19
	uint	dm_var_stat_ratio_0				:	 4;					// 20:23
	uint	dm_var_stat_ratio_1				:	 4;					// 24:27
	uint	dm_var_lg_range					:	 4;					// 28:31
}	DI_ENGINE4;

typedef union	{
	uint			nDI_ENGINE4;
	DI_ENGINE4		bDI_ENGINE4;
}	DI_ENGINE4_U;

typedef struct	{
	uint	pd_thres_pulldown_cnts				:	10;
	//uint	pd_thres_weight						:	 3;
	//uint										:	 2;
	uint	pd_thres_weight						:	 5;
	uint	pd_thres_user_val_disable			:	 1;
	uint	pd_user_checkout					:	 2;
	uint										:	 2;
	uint	pd_cnts_thres_pulldown_checkout		:	 4;
	//uint										:	 8;
	uint	pd_cnts_zero_prevent				:	 1;
	uint										:	 7;
}	DI_PD_THRES0;
...
typedef struct	{
	uint	pd_gen_thres_judder_div_half	: 	 1;
	uint									: 	 7;
	uint	pd_judder_thres_two_lbufs		:	 8;
	//uint									: 	16;
	uint	pd_not_cnt_judder_stati			: 	 1;
	uint	pd_mul_dm_thres					: 	 7;
	uint									: 	 8;
}	DI_PD_JUDDER_MISC;

typedef	union	{
	uint				nDI_PD_JUDDER_MISC;
	DI_PD_JUDDER_MISC	bDI_PD_JUDDER_MISC;
}	DI_PD_JUDDER_MISC_U;
...
typedef	union	{
	uint			nDI_PD_CNT_JD_FRM;
}	DI_PD_CNT_JD_FRM_U;

typedef struct	{
	uint	idx_current						:	 4;
	uint	min_idx_cur_0					:	 4;
	uint	min_idx_cur_1					:	 4;
	uint	min_idx_cur_2					:	 4;
	uint	min_idx_2nd_0					:	 4;
	uint	min_idx_2nd_1					:	 4;
	uint	min_idx_2nd_2					:	 4;
	uint									:	 4;
}	DI_PD_IDX_STATUS;

typedef	union	{
	uint				nDI_PD_IDX_STATUS;
	DI_PD_IDX_STATUS	bDI_PD_IDX_STATUS;
}	DI_PD_IDX_STATUS_U;
...
typedef struct 	{
	uint	encoded_data_cnt				:	20;
	uint	undef0							:	12;
}	ENCODED_DATA;

typedef union 	{
	uint			nENCODED_DATA;
	ENCODED_DATA	bENCODED_DATA;
}	ENCODED_DATA_U;
...
typedef struct	{
	...
	uint	y2r_en							:	 1;			// 09
	uint									:	 2;			// 10:11
	uint	y2r_mode						:	 2;			// 12:13
	uint	r2y_mode						:	 2;			// 14:15
	uint									:	15;			// 16:30
...
typedef	volatile struct {
	...
	DI_ENGINE2_U			uDI_ENGINE2;			// 0x08c
	DI_ENGINE3_U			uDI_ENGINE3;			// 0x090
	DI_ENGINE4_U			uDI_ENGINE4;			// 0x094
	DI_PD_THRES0_U			uDI_PD_THRES0;			// 0x098
	DI_PD_THRES1_U			uDI_PD_THRES1;			// 0x09c
	DI_PD_JUDDER_U			uDI_PD_JUDDER; 			// 0x0a0
	DI_PD_JUDDER_MISC_U		uDI_PD_JUDDER_MISC; 	// 0x0a4
	DI_MISC_CTRL_U			uDI_MISC_CTRL; 			// 0x0a8
	DI_STATUS_U				uDI_STATUS; 			// 0x0ac
	DI_PD_STATUS_U			uDI_PD_STATUS; 			// 0x0b0
	DI_REGION_0_U			uDI_REGION_0;			// 0x0b4
	DI_REGION_1_U			uDI_REGION_1;			// 0x0b8
	...
	DI_PD_CNT_JD_FRM_U		uDI_PD_CNT_JD_FRM;		// 0x0d8
	DI_PD_IDX_STATUS_U		uDI_PD_IDX_STATUS;		// 0x0dc
	uint					undef_1[8];				// 0x0e0 ~ 0x0fc
	...
	RD_END_ADDR_U			uRD_END_ADDR0;			// 0x1e0
	RD_END_ADDR_U			uRD_END_ADDR1;			// 0x1e4
	RD_END_ADDR_U			uRD_END_ADDR2;			// 0x1e8
	RD_ROLL_MODE_U			uRD_ROLL_MODE;			// 0x1ec
	RD_CUR_ADDR_U			uRD_CUR_ADDR0;			// 0x1f0
	RD_CUR_ADDR_U			uRD_CUR_ADDR1;			// 0x1f4
	RD_CUR_ADDR_U			uRD_CUR_ADDR2;			// 0x1f8
	uint					undef_4_1[1];			// 0x1fc
	...
	ENCODED_DATA_U			uENCODED_DATA0;			// 0x230
	ENCODED_DATA_U			uENCODED_DATA1;			// 0x234
	ENCODED_DATA_U			uENCODED_DATA2;			// 0x238
	uint					undef_5_1[1];				// 0x23c
	WD_END_ADDR_U			uWD_END_ADDR0;			// 0x240
	WD_END_ADDR_U			uWD_END_ADDR1;			// 0x244
	WD_END_ADDR_U			uWD_END_ADDR2;			// 0x248
	WD_ROLL_MODE_U			uWD_ROLL_MODE;			// 0x24c
	WD_CUR_ADDR_U			uWD_CUR_ADDR0;			// 0x250
	WD_CUR_ADDR_U			uWD_CUR_ADDR1;			// 0x254
	WD_CUR_ADDR_U			uWD_CUR_ADDR2;			// 0x258
	uint					undef_5_2[9];			// 0x25c ~ 0x27c
