...
static int claa070np01_set_power(struct lcd_panel *panel, int on, unsigned int lcd_num)
{
	PDDICONFIG	pDDICfg;
...
struct lcd_panel claa070np01_panel = {
	.name		= "CLAA070NP01",
	.manufacturer	= "CHUNGHWA",
	.id		= PANEL_ID_CLAA070NP01,
	.xres		= 1024,
	.yres		= 600,
	.width		= 153,
	.height		= 90,
...
static int claa070np01_probe(struct platform_device *pdev)
{
	...
	claa070np01_panel.dev = &pdev->dev;
	#if defined(CONFIG_ARCH_TCC892X)
	...
	#endif
	clk_enable(lvds_clk);	
	tccfb_register_panel(&claa070np01_panel);
	return 0;
}

static int claa070np01_remove(struct platform_device *pdev)
{
	return 0;
}

static struct platform_driver claa070np01_lcd = {
	.probe	= claa070np01_probe,
	.remove	= claa070np01_remove,
	.driver	= {
...
static __init int claa070np01_init(void)
{
	printk("~ %s ~ \n", __func__);
	return platform_driver_register(&claa070np01_lcd);
}

static __exit void claa070np01_exit(void)
{
	platform_driver_unregister(&claa070np01_lcd);
}

subsys_initcall(claa070np01_init);
module_exit(claa070np01_exit);
