...
static BlendStruct BlendTable[BLENDMODE_INVALID][BLENDTABLE_LEN] = 
{
	...
	{ //       min                        max          Y/C   vt_f  coef
		{0,                     10 +BLENDTABLE_OFFSET, 0x22, 0x06, 0x0a3}, // index_0
		{10 +BLENDTABLE_OFFSET, 25 +BLENDTABLE_OFFSET, 0x22, 0x0f, 0x0a3}, // index_1
		{25 +BLENDTABLE_OFFSET, 35 +BLENDTABLE_OFFSET, 0x20, 0x15, 0x0a3}, // index_2
		{35 +BLENDTABLE_OFFSET, 45 +BLENDTABLE_OFFSET, 0x20, 0x1d, 0x0c2}, // index_3
		{45 +BLENDTABLE_OFFSET, 55 +BLENDTABLE_OFFSET, 0x2E, 0x25, 0x0c2}, // index_4
		{55 +BLENDTABLE_OFFSET, 65 +BLENDTABLE_OFFSET, 0x2E, 0x2f, 0x0c2}, // index_5
		...
		{85 +BLENDTABLE_OFFSET, 100+BLENDTABLE_OFFSET, 0x4A, 0x3f, 0x0e1}, // index_8
	...
	{
		{0,                     10 +BLENDTABLE_OFFSET, 0x22, 0x06, 0x0a3}, // index_0
		{10 +BLENDTABLE_OFFSET, 20 +BLENDTABLE_OFFSET, 0x22, 0x0f, 0x0a3}, // index_1
		{20 +BLENDTABLE_OFFSET, 30 +BLENDTABLE_OFFSET, 0x20, 0x15, 0x0a3}, // index_2
		{30 +BLENDTABLE_OFFSET, 35 +BLENDTABLE_OFFSET, 0x20, 0x1d, 0x0c2}, // index_3
		{35 +BLENDTABLE_OFFSET, 45 +BLENDTABLE_OFFSET, 0x2E, 0x25, 0x0c2}, // index_4
		{45 +BLENDTABLE_OFFSET, 55 +BLENDTABLE_OFFSET, 0x2E, 0x2f, 0x0c2}, // index_5
		...
		{75 +BLENDTABLE_OFFSET, 85 +BLENDTABLE_OFFSET, 0x4A, 0x3f, 0x0e1}, // index_8
    ...
    // BLENDMODE_NTSCM
	{
		...
		{10 +BLENDTABLE_OFFSET, 20 +BLENDTABLE_OFFSET, 0x22, 0x0f, 0x0a3}, // index_1
		{20 +BLENDTABLE_OFFSET, 30 +BLENDTABLE_OFFSET, 0x20, 0x15, 0x0a3}, // index_2
		{30 +BLENDTABLE_OFFSET, 40 +BLENDTABLE_OFFSET, 0x20, 0x1d, 0x0c2}, // index_3
		{40 +BLENDTABLE_OFFSET, 50 +BLENDTABLE_OFFSET, 0x2E, 0x25, 0x0c2}, // index_4
		{50 +BLENDTABLE_OFFSET, 60 +BLENDTABLE_OFFSET, 0x2E, 0x2f, 0x0c2}, // index_5
		...
		{80 +BLENDTABLE_OFFSET, 89 +BLENDTABLE_OFFSET, 0x4A, 0x3f, 0x0e1}, // index_8
...
#define CHANNEL_FILTER_SAMP_NUM      9
...
static int InvalidChannelFilter(void)
{
	...
	RDAEXT_DebugPrint("[rdamtv]InvalidChannel, temp = %d, var = %d, %d, %d, %d, %d, %d, %d\n",
		temp, var[0],var[1],var[2],var[3],var[4],var[5],var[6]);
	
	if (temp > CHANNEL_FILTER_THRESHOLD)
	{
		rt = RDAMTV_RT_SCAN_FAIL;
		RDAEXT_DebugPrint("[rdamtv]InvalidChannel done!!\n");
	}
	else if (temp == 0)
	{
...
static uint8 SetChannelAux(uint32 nChannel, uint32 nFlag)
{
	...
	{
		RDA5888_ScanChannel(p_rdamtv_cur_map[i].freq, p_rdamtv_cur_map[i].chstd, nFlag); 
	...
	{
		RDAEXT_DebugPrint("SetChannelAux, channel not exist");
...
static void CheckSignalLevel(void)
{
	...
	if (1 == g_nVideoEnhanceEnable)
	{
		...
		// dynamic frame rate process. (for mtk.)
...
{
    RDAEXT_DebugPrint("[rdamtv]CHIP_VER: %d, LIB_VER: %d.%d.%d \n", RDA5888_CHIP_VER, 
...
void RDAAPP_TimerHandler(void)
{
	...
	else if (m_nIsTPMode)
	{
		...
		//RDA5888_FieldWriteReg(0x1b1, 0x0008, 0);
		m_nIsTPMode = 0;
	}
#endif
...
void RDAAPP_SetChannelMap(p_rdamtv_channel_t pstChMap, uint32 nWorkMode, uint32 nVidStd)
{
	...
	RDAEXT_DebugPrint("RDAAPP_SetChannelMap, channel counter = %d\n", g_nCurChCnt);
}
...
void RDAAPP_SetTVArea(uint32 cTVArea)
{
	...
	RDAEXT_DebugPrint("RDAAPP_SetTVArea, tv_area=%d \n", m_nCurTvArea);
...
uint8 RDAAPP_SetChannel(uint32 nChannel, uint32 nFlag)
{
	...
	{
	    RDAEXT_DebugPrint("[rdamtv]RDAAPP_SetChannel, fail! nChannel = %d\n", nChannel);
		m_nIsTPMode = 1;
#if ((RDA5888_CHIP_VER == 4) ||(RDA5888_CHIP_VER == 5))
		RDA5888_ReadReg(0x130, &m_nTPRegBak);
		RDA5888_WriteReg(0x130, 0x803);
	...
	{
	    RDAEXT_DebugPrint("[rdamtv]RDAAPP_SetChannel, done! nChannel = %d\n", nChannel);
		m_nIsTPMode = 0;
		rt = TRUE;
	}
