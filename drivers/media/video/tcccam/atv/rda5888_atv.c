...
#ifdef CONFIG_VIDEO_ATV_SENSOR_RDA5888

#define	RDA5888_ATV_WIDTH				360
#define	RDA5888_ATV_HEIGHT				240
...
{
	{RDA5888_ATV_WIDTH, RDA5888_ATV_HEIGHT},
	{ RDA5888_ATV_WIDTH, RDA5888_ATV_HEIGHT}
};
...
#endif
#else
//#error Select RDA5888_ATV init mode!
#endif
	{REG_TERM, VAL_TERM}
};
...
static int sensor_open(void)
{
	...
	RDAEXT_DebugPrint("rdamtv_if_Init start2\n");
...
{
	int nCntOfChannel = 1;
	unsigned char bIsFindChaanel;

	
	
	RDAAPP_SetTVArea(RDAMTV_AREA_KOREA);

	#if 1
		//RDA5888_SetTestPattern();
		RDAAPP_SetChannel(10, 0);
	#else
		for(nCntOfChannel; nCntOfChannel<=68; nCntOfChannel++){
			bIsFindChaanel = RDAAPP_SetChannel(nCntOfChannel, 1);
			if(bIsFindChaanel)
				printk("Normal Working Channel is [%d] \n", nCntOfChannel);
		}
	#endif
...
#endif /*CONFIG_VIDEO_CAMERA_SENSOR_RDA5888_ATV*/
