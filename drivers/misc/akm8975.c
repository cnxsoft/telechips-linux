...
static DECLARE_WAIT_QUEUE_HEAD(int_ready_wq);  // for preemptive

static atomic_t data_ready;
static atomic_t open_count;
static atomic_t open_flag;
static atomic_t reserve_open_flag;
static atomic_t int_enable_flag; // for preemptive
...
static int AKECS_SetMode(char mode)
{
	...
	switch (mode) {
		case AK8975_MODE_SNG_MEASURE:
			// for preemptive
			if(!atomic_read(&int_enable_flag))
			{
				wait_event_interruptible_timeout(int_ready_wq,
							 atomic_read(&int_enable_flag), 300);
			}
...
static int AKECS_GetData(char *rbuf, int size)
{
	...
	wait_event_interruptible_timeout(data_ready_wq,
									 atomic_read(&data_ready), 100); // 1000 -> 100
...
static short *AKECS_GetMetrix(void)
{
	return (short *)&layoutMatrix[0]._11;
}
		   ...
		   unsigned long arg)
{
	...
	switch (cmd) {
		...
		case ECS_IOCTL_GET_SUSPEND_STATUS:
			AKMFUNC("ECS_IOCTL_GET_SUSPEND_STATUS");
			status = atomic_read(&suspend_flag);
			break;
		case ECS_IOCTL_GET_MATRIX:
			AKMFUNC("ECS_IOCTL_GET_MATRIX");
			pData = AKECS_GetMetrix();
			break;
		default:
			return -ENOTTY;
	}
	...
	switch (cmd) {
		...
		case ECS_IOCTL_GET_MATRIX:
			if(pData == NULL)
			{
				AKMDBG("get metrix failed.");
				return -EFAULT;
			}
			if (copy_to_user(argp, pData, sizeof(layoutMatrix))) {
				AKMDBG(" copy_to_user failed(ECS_IOCTL_GET_MATRIX).");
				return -EFAULT;
			}
			break;			
		default:
...
int akm8975_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	init_waitqueue_head(&int_ready_wq);
