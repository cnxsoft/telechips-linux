/*
 * linux/sound/soc/tcc/tcc_board_wm8993.c
 *
...
static int tcc_hw_params(struct snd_pcm_substream *substream, struct snd_pcm_hw_params *params)
{
	...
	ret = snd_soc_dai_set_sysclk(codec_dai, WM8993_SYSCLK_MCLK, clk,
		SND_SOC_CLOCK_IN);
	if (ret < 0)
		return ret;
...
static const struct snd_kcontrol_new wm8993_tcc_controls[] = {
 ...
 */
static int tcc_wm8993_init(struct snd_soc_codec *codec)
{
	int i, err;

	/* Add tcc specific controls */
	for (i = 0; i < ARRAY_SIZE(wm8993_tcc_controls); i++) {
		err = snd_ctl_add(codec->card,
			snd_soc_cnew(&wm8993_tcc_controls[i],codec, NULL));
		if (err < 0)
			return err;
	}
...
static int tcc_board_probe(struct platform_device *pdev)
{
	...
	tca_ckc_setiobus(RB_DAI0CONTROLLER, 1);
	tca_ckc_setiobus(RB_ADMA0CONTROLLER, 1);
...
static struct wm8993_setup_data tcc_wm8993_setup = {
    .i2c_bus = 2,
...
static int __init tcc_init(void)
{
    ...
    //tcc_pca953x_setup(PCA9539_U1_SLAVE_ADDR, CODEC_ON, OUTPUT, HIGH, SET_DIRECTION | SET_VALUE);
    /*
     * 1: LCD_ON
     * 2: CODEC_ON
     * 3: SD0_ON
     ...
     * 8: BT_ON
     */
    gpio_set_value(TCC_GPEXT2(1), 0);   /* codec un-Mute */
    gpio_set_value(TCC_GPEXT1(2), 1);   /* codec on */
    gpio_set_value(TCC_GPEXT1(6), 1);   /* external codec on */
    printk("tcc_init(): call gpio_set_value() \n");

//    i2c_register_board_info(2, i2c_device, ARRAY_SIZE(i2c_device));
