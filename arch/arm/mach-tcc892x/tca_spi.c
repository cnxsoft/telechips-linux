...
static int tca_spi_set_port(struct tca_spi_handle *h)
{
	...
	switch (h->gpsb_port) {
		...
		case 13:
		{
		    tcc_gpio_config(TCC_GPE(28), GPIO_FN(4));
		    tcc_gpio_config(TCC_GPE(29), GPIO_FN(4));
		    tcc_gpio_config(TCC_GPE(30), GPIO_FN(4));
...
static int tca_spi_clear_port(struct tca_spi_handle *h)
{
	...
	switch (h->gpsb_port) {
		...
		case 13:
		{
		    tcc_gpio_config(TCC_GPE(28), GPIO_FN(0)|GPIO_OUTPUT|GPIO_LOW);
...
unsigned int TCCREFTIME_GetTime(void)
{
	if(pTCCTIMER == NULL)
		return 0;	
    guiRefTime = pTCCTIMER->TC32MCNT.nREG/6000; //because 6000Khz, divided by 6000 needs
