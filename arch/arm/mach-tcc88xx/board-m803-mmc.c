...
int m803_mmc_card_detect(struct device *dev, int id)
{
	BUG_ON(id > TCC_MMC_TYPE_MAX);
...
int m803_mmc_suspend(struct device *dev, int id)
{
	if(id == TCC_MMC_TYPE_WIFI) {
...
int m803_mmc_set_power(struct device *dev, int id, int on)
{
	//if (on) {
	//} else {
	//}
...
int m803_mmc_set_bus_width(struct device *dev, int id, int width)
{
	return 0;
}
...
struct tcc_mmc_platform_data m803_mmc_platform_data[] = {
	[TCC_MMC_TYPE_SD] = {
		.slot	= 5,
		...
		.init	= m803_mmc_init,
		.card_detect = m803_mmc_card_detect,
		.cd_int_config = m803_mmc_cd_int_config,
		.suspend = m803_mmc_suspend,
		.resume	= m803_mmc_resume,
		.set_power = m803_mmc_set_power,
		.set_bus_width = m803_mmc_set_bus_width,
...
static int __init m803_init_mmc(void)
{
	if (!machine_is_m803())
		return 0;
	...
	// system_rev : 0x0012 = M803 V0.2A
	if(system_rev == 0x0012 || system_rev == 0x0013)
	{
		// Change Card Detect
	...
	tcc_sdhc1_device.dev.platform_data = &m803_mmc_platform_data[1];
	platform_device_register(&tcc_sdhc1_device);
#endif
#if defined(CONFIG_MMC_TCC_SDHC2)
	tcc_sdhc2_device.dev.platform_data = &m803_mmc_platform_data[2];
