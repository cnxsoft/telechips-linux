/* linux/arch/arm/mach-tcc88xx/board-tcc8800-panel.c
 *
...
static struct led_classdev tcc8800_backlight_led = {
	.name		= "lcd-backlight",
	.brightness	= DEFAULT_BACKLIGHT_BRIGHTNESS,
	.brightness_set	= tcc8800_brightness_set,
};

static int tcc8800_backlight_probe(struct platform_device *pdev)
{
	...
	return led_classdev_register(&pdev->dev, &tcc8800_backlight_led);
}
...
static struct platform_driver tcc8800_backlight_driver = {
	.probe	= tcc8800_backlight_probe,
	.remove	= tcc8800_backlight_remove,
	.driver	= {
		.name	= "tcc8800-backlight",
		.owner	= THIS_MODULE,
	},
};

static struct platform_device tcc8800_backlight = {
	.name = "tcc8800-backlight",
};

int __init tcc8800_init_panel(void)
{
	int ret;
	...
	platform_device_register(&tcc8800_backlight);
	ret = platform_driver_register(&tcc8800_backlight_driver);
	if (ret)
		return ret;
...
device_initcall(tcc8800_init_panel);
